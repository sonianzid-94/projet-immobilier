...
function filtrerParType(type) {
  ...
  // Filtre par types
  if (typeActif) {
    resultat = resultat.filter(maison => maison.type === typeActif);
  }
  housesToDisplay = resultat; // On stocke le résultat filtré
  currentPage = 1; // On revient à la page 1
  afficherMaisons(housesToDisplay);
...
function filtrercommune(commune) {
  let typeActif = commune;

  let resultat = maisons;

  // Filtre par types
  if (typeActif) {
    resultat = resultat.filter(maison => maison.commune.toLowerCase() === typeActif.toLowerCase()
);
  }
  if (resultat.length >= 1) {
    ...
    const container = document.getElementById("listeMaisons");
    const parag = document.getElementById("paragraphe");
  container.innerHTML = "";
  parag.innerHTML = `
  <p class="  text-2xl font-bold text-center "> Aucune maison disponible dans cette commune </p>
  `;
  }
...
window.onload = () => {
  ...
  if (searchQuery) {
    console.log("Filtrage pour :", searchQuery);
    filtrercommune(searchQuery);
  } else {
